...
const SEVERITY_STYLES: Record<string, string> = {
  low: 'bg-indigo-subtle text-indigo-700',
  medium: 'bg-teal-subtle text-teal-600',
...
const ITEM_CONDITION_STYLES: Record<string, string> = {
  good: 'bg-badge-resolved-bg text-badge-resolved-text',
  damaged: 'bg-teal-subtle text-teal-600',
  missing: 'bg-badge-danger-bg text-badge-danger-text',
};
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3">
              <ClipboardIcon className="h-6 w-6 text-text-secondary" />
          ...
          {report.events_created > 0 && (
            <span className="inline-flex items-center rounded-pill bg-badge-danger-bg px-3 py-1 text-sm font-medium text-badge-danger-text">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-card-border">
                  <th className="pb-3 text-left font-medium text-text-muted">Ítem</th>
                  <th className="pb-3 text-center font-medium text-text-muted">Esperado</th>
                  <th className="pb-3 text-center font-medium text-text-muted">Actual</th>
                  <th className="pb-3 text-center font-medium text-text-muted">Condición</th>
                  <th className="pb-3 text-left font-medium text-text-muted">Notas</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-card-border">
                {reportData.inventory.map((item, idx) => (
                  <tr key={idx}>
                    <td className="py-3 font-medium text-text-primary">{item.item_name}</td>
                    <td className="py-3 text-center text-text-secondary">{item.expected_count ?? '—'}</td>
                    <td className="py-3 text-center text-text-secondary">{item.actual_count ?? '—'}</td>
                    <td className="py-3 text-center">
                      <span className={`inline-flex items-center rounded-pill px-2.5 py-0.5 text-xs font-medium ${ITEM_CONDITION_STYLES[item.condition] || 'bg-indigo-subtle text-indigo-700'}`}>
                        {ITEM_CONDITION_LABELS[item.condition] || item.condition}
                      </span>
                    </td>
                    <td className="py-3 text-text-muted">{item.notes || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-card border border-card-border bg-white p-6 shadow-card">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {reportData.damages.map((damage, idx) => (
              <div key={idx} className="rounded-card border border-card-border bg-slate-50/50 p-4">
                ...
                <p className="text-sm text-text-secondary">{damage.description}</p>
                {damage.photo_index != null && (
                  <p className="mt-2 text-xs text-text-muted">Foto #{damage.photo_index + 1}</p>
                )}
              </div>
            ))}
          </div>
